...
def recieve_intercom_hook(request):
    if(request.data['topic'] == 'conversation.admin.replied'):
        return Response({"data": "Denied hook admin reply"})
    ...
    message_detail = forward_message_to_mattermost(
        token_mattermost, root_message_id, message)
    if(root_message_id == ''):
